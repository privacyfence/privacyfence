...
# `runtime-windows.lock.txt`-style split or an unpinned exception for one package. There's
# therefore no need to pin the compiling Python's own minor version to CI's either (the older
# pip-tools-based version of this script did, for a related but different reason -- resolving
# `python_version`-gated markers consistently -- that a universal resolution also makes moot: it
# resolves markers for the full `requires-python` range from pyproject.toml, not just the
# invoking interpreter's own version).
#
...
# against.
uv pip compile --universal --generate-hashes --no-strip-extras \
