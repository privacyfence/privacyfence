...
AWAIT_APPROVAL_TOOL = types.Tool(
    ...
    description=(
        ...
        "plan.md §5), and report their status -- status only, never content. Before your first call "
        "to this tool for a given approval, make sure the human has actually been told: relay that "
        "result's own message and url to the user in your reply so they know a decision is waiting "
        "on them -- do not call this (or anything else) silently first. Pass every approval_id "
