...
| Installer/package correctness | Native artifact CI — the three `*_packaged_*`/`*_lifecycle` modules in `build.yml`, plus the two graphical-session workflows |
...
(Phase 0), which built out every row above and was retired once all of its phases had landed. This
...
(the now-removed `automated-test-strategy-plan.md` Phase 2.3), added as its own marker rather than reusing `system`
...
exceptions; otherwise it's a sign the corresponding phase in the now-removed `automated-test-strategy-plan.md`
...
automation isn't done yet, the corresponding phase in the now-removed `automated-test-strategy-plan.md` is what
...
be required (the now-removed `automated-test-strategy-plan.md` Phase 11) — update it in the same PR whenever
...
- `tests/integration/test_mcp_daemon_contract.py` — spawns a real `privacyfence.web.server.WebServer`
  ...
  test-only one.
- `tests/system/test_local_mode_system.py` (the now-removed `automated-test-strategy-plan.md` Phase 3, layer 3
...
- `tests/integration/test_browser_smoke.py` (TST-06) — drives the real web approval surface in a
  ...
  external credential, unlike the connector accounts §0/§2.1 need. The now-removed `automated-test-strategy-plan.md`
...
- `--check [connector ...]` — calls each connector's read methods against its seed artifact,
  ...
  The now-removed `automated-test-strategy-plan.md` Phase 1 residual work) — a quick signal for whether a
...
- `--lifecycle [connector ...]` — 1.8 (the now-removed `automated-test-strategy-plan.md` Phase 1 residual
...
says should stay manual. The now-removed `automated-test-strategy-plan.md` Phase 5 cross-checked how much of what
...
all three OSes (the now-removed `automated-test-strategy-plan.md` Phase 3, `tests/system/test_local_mode_system.py`)
has landed too. Layer 6 (packaged-artifact) is now also fully settled — see the taxonomy table
above (the now-removed `automated-test-strategy-plan.md` Phase 6, done): all three platforms' install/lifecycle/
