...
**Extended by [ADR 0028](0028-clients-without-the-shim-get-capability-urls.md), 2026-09-23
([#611](https://github.com/privacyfence/privacyfence/pull/611)).** Callers with no shim in their
request path (Claude Code, org mode, other direct HTTP clients) get single-use capability URLs for
uploads and downloads, replacing the no-bridge fallback's upload error and bearer-authenticated
download link described below. The "Non-shim clients" item under Out of scope is what 0028 does.
...
### Clients without the bridge

Recorded separately as [ADR 0028](0028-clients-without-the-shim-get-capability-urls.md): upload
slots and capability download links for callers with no shim in their request path.
...
- `tests/unit/test_local_files.py`, `tests/unit/test_upload_staging.py`,
  `tests/unit/web/test_routes_file_bridge.py`: the Python half of the protocol, including the
...
- Connector-level tests (extended `test_drive_connector.py`/`test_gmail_connector.py`/`test_confluence_connector.py`): with the
...
- `local-mode-fixes-plan.md` (never merged to `main`; read it with
  `git show 453ae02e:local-mode-fixes-plan.md`) -- the working plan this ADR and its implementation
  were written from; Phases 2-4 there are the Out-of-scope items above, spelled out in full. Their
  decisions are ADRs [0026](0026-the-companion-manages-the-daemon-through-the-service-manager.md)
  and [0027](0027-a-group-member-cannot-take-over-another-members-companion-socket.md) (Phase 2),
  [0008](0008-one-principal-per-os-user.md) (Phase 3) and
  [0028](0028-clients-without-the-shim-get-capability-urls.md) (Phase 4).
