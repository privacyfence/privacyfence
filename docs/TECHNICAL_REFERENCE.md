...
- `privacyfence_status` (issue #396) — the one meta-tool guaranteed to answer even when `connectors == []` leaves every other tool missing, so an empty or partial tool list reads as "not set up yet", not "nothing to do here". Returns `{mode, setup_complete, connectors, next_step, message}` and, in local mode when un-onboarded, `sign_in_url`: `mode` is `"local"` or `"org"`; `connectors` is a list of `{name, enabled, authenticated, blocked_by}` (`blocked_by` is `null` once authenticated or deliberately disabled, otherwise `"no_org_config"`, `"not_authenticated"`, or a short redacted reason — `SettingsController.status_connectors`, sourced from `build_connectors()`'s own per-connector failure map); `setup_complete` is `true` once at least one connector is authenticated. When not yet set up, local mode mints a fresh SEC-06 sign-in link straight to `/settings/connectors` (Part C — the screen that actually unblocks the user, not `/settings`'s own General default) the same way `privacyfence_get_sign_in_link(page="connectors")` does, but caches it in-process for a few minutes instead of re-minting (and rewriting the discovery file) on every call; organization mode never has a `sign_in_url` and instead points the human at their administrator. Wired via `McpDispatcher.set_connectors_state_provider`, a seam parallel to `set_bootstrap_link_provider` above.
