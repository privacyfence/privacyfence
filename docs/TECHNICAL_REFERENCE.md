...
- `privacyfence_create_upload_slot` ([ADR 0028](adr/0028-clients-without-the-shim-get-capability-urls.md)) — mints an `upload_staging.UploadStagingStore` slot for a client with no `.mcpb` extension (Claude Code, any other direct HTTP client, or org mode) and returns `{upload_id, upload_url, method: "PUT", max_bytes, expires_at, example}`: `upload_url` is a capability URL (`/mcp-files/slots/<token>`) needing no `Authorization` header at all, since the token in the URL is itself the one-time credential. The caller `PUT`s the file's bytes there, then passes `upload_id` back as `drive_upload_file`'s own `upload_id` parameter, or as an `"upload:<upload_id>"` entry in a `gmail_*_with_attachments` tool's `attachments` array — claimed the ordinary, principal-checked way at that point (`local_files.require_local_files`'s `upload:` handling), so filling a slot needs no principal but claiming it still does. No gate, no approval, no side effect beyond minting the slot — approval happens when the file's actual destination tool runs, exactly as for a bridge-fetched `local_path`.
