...
- `web.settings.enabled: true` (default) — turns on `GET /settings` and its
  ...
  `settings_window_html.py`'s own module docstring). It's the screen `privacyfence_status` sends an un-onboarded
  install's human to, and where the companion's own Open Settings item lands them, so they arrive
  at the screen that actually unblocks the install rather than `/settings`'s own General page. That page also renders a short, dismissible welcome banner (client-side,
