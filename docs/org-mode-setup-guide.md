...
`/settings` in org mode (#400) is a read-only view scoped very differently from local mode's combined settings page — it does **not** mount that page's ~30-action editor (connector management, the update banner, Telegram's interactive auth stay local-mode-only, see `web/org_settings_scope.py`'s `NOT_APPLICABLE_ACTIONS`). What it does give every signed-in principal, linked from `/approvals`'s own footer:
