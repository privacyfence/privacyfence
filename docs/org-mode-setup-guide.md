...
- **`GET /settings`** — that principal's own auto-accept rules and trusted-resource grants: a Remove button per row, and an "Add a rule" form (an operation/rule-type picker constrained to the same fixed menu local mode's own picker uses, plus a value field) that adds a new one. Both the add and every removal go through the same `org_session` CSRF/origin checks as `/approvals`, and are written to the audit log. No principal can see, add to, or remove another's rules from here.
...
Beyond adding and removing a rule row, editing `/settings`'s per-principal rules further (changing an existing row's value in place) and grants (adding, removing, or editing a trusted resource) from the browser is still out of scope; `/settings/privacy`'s policy fields, described above, can be edited directly. Each of the two settings below is configured, and takes effect, differently.
